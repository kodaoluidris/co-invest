...
export default {
    ...
    mounted() {
        ...
        for(let i =0; i < this.main_property.groups;i++) {
            let ii=i +1;
            this.data.push({gn:'Group ' + ii,np:'', price:this.main_property.price})
        }
    },
    methods:{
        closeMe() {
            this.$bvModal.hide("groups");
        },
        save() {
            ...
            // if(status) {
                ...
                axios
                .post(this.dynamic_route('/main_properties/manage_groups'), payload, {
                    headers:{
